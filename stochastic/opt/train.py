...
def train_svrg(params, xds, data_chan_freq, batch_size, outdir, error_fn, LR, *opt_args, **extra_args):
    ...
    for epoch in range(EPOCHS):
        ...
        logger.info("Epoch {} in {} secs, min, mean and final loss are {:.2e} {:.2e} and {:.2e}, iter {}".format(epoch, epoch_t, best_loss, mean_loss, loss_i, opt_info[0]))
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in range(num_batches):
            ...
            if extra_args["noneg"]:
                ...
                params = jaxGrads.get_params(opt_state)
            xk, _ = ravel_pytree(params)
            eps = np.linalg.norm(xk - x0) / np.linalg.norm(xk)
            if eps < DELTA_LOSS:
                CONV = True 
