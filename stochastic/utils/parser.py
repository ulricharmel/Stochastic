...
def init_learning_rates(lr):
    ...
    args:
        lr (list)
    returns:
        dictionary (stokes, radec, shape_params)
    """

    assert len(lr)==1 or len(lr) == 3, "Either set a constant learning rate or set a different learning rate for each parameter"

    if len(lr) == 1:
        return dict(stokes=float(lr[0]), radec=float(lr[0]), shape_params=float(lr[0]))
    else:
        return dict(stokes=float(lr[0]), radec=float(lr[1]), shape_params=float(lr[2]))
